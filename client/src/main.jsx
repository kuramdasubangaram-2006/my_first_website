...
function ConsentPage({ token }) {
  ...
  const allow = async () => {
    ...
    try {
      ...
      setError('We could not complete this consent session.');
      setStatus('ready');
    }
  };

  const decline = async () => {
    await api(`/tracking/${token}/decline`, { method: 'POST' });
    setStatus('declined');
  };
  ...
  if (status === 'complete' || status === 'declined')
    return (
      <main className="consent-shell">
        <div className="consent-card centered">
          ...
          <p>
            {status === 'complete'
              ? 'Your consented session has been recorded. You can close this page.'
              : 'Your decision was recorded and the session will remain empty.'}
          </p>
        </div>
      </main>
    );
    ...
    <main className="consent-shell">
      <div className="consent-card">
        <div className="eyebrow">
          <ShieldCheck size={16} /> INFORMATION KB
        </div>

        <h1>Choose what to share.</h1>

        <p className="lead">
          This page asks for permission before collecting limited information
          about your visit. You are in control, and declining will not affect
          anything else.
        </p>
        ...
        <div className="collection-list">
          <Info
            ...
            copy="Used to estimate a general city, region, and country."
          />

          <Info
            icon={<MapPin />}
            title="Approximate location"
            copy="Derived from your IP address and never treated as precise GPS."
          />

          <Info
            icon={<MapPin />}
            title="Optional GPS location"
            copy="Requested only after you allow this page to continue, through your browser's permission prompt."
          />

          <Info
            icon={<MonitorSmartphone />}
            title="Browser and device details"
            copy="Browser, operating system, device type, display size, language, time zone, and referrer."
          />
        </div>
        ...
        <div className="notice">
          <LockKeyhole size={18} />
          <span>
            We do not collect passwords, cookies, tokens, contacts, files,
            camera, microphone, or any other private content.
          </span>
        </div>
        ...
        <small>
          By continuing, you agree to the collection described above. Data is
          retained for 90 days and can be deleted by the administrator.
        </small>
      </div>
    </main>
  );
