...
const lookupWhoisFreaks = async (ip) => {
  if (!process.env.WHOISFREAKS_API_KEY) return {};

  try {
    const url = new URL('https://api.whoisfreaks.com/v1.0/security');
    url.searchParams.set('apiKey', process.env.WHOISFREAKS_API_KEY);
...
const clientIp = (forwardedFor ? forwardedFor.split(',')[0].trim() : req.ip).replace('::ffff:', ''); const lookupIp = await resolveLocationIp(clientIp).catch(() => clientIp);const geo = await lookupIpLocation(lookupIp).catch(() => ({}));const gpsLocation = await lookupGpsLocation(payload.latitude, payload.longitude); const reputation = await lookupWhoisFreaks(lookupIp);console.log('WhoisFreaks reputation:', reputation); const data = {whoisfreaks_reputation: reputation,ram: payload.ram || null,storage: payload.storage || null,
storage: payload.storage || null, ...browserInfo(ua), ip_address: geo.ip || lookupIp, ip_city: geo.city || geo.city_name || null, ip_region: geo.region || geo.region_name || null, ip_country: geo.country_name || geo.country || geo.country_code || null, ip_metadata: {
  ...(geo.metadata || {}),
  gpsLocation
