...
const clientIp = (forwardedFor ? forwardedFor.split(',')[0].trim() : req.ip).replace('::ffff:', ''); const lookupIp = await resolveLocationIp(clientIp).catch(() => clientIp); const geo = await lookupIpLocation(lookupIp).catch(() => ({})); const data = { ...browserInfo(ua), ip_address: geo.ip || lookupIp, ip_city: geo.city || geo.city_name || null, ip_region: geo.region || geo.region_name || null, ip_country: geo.country_name || geo.country || geo.country_code || null, ip_metadata: geo.metadata || null, latitude: payload.latitude ?? null,
