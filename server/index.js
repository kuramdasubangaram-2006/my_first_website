...
app.post('/api/tracking-links', admin, async (_req, res) => { const token = crypto.randomBytes(24).toString('base64url'); await createSession(hashToken(token)); res.status(201).json({ token, path: `/track/${token}` }); });
...
app.get('/api/sessions', admin, async (req, res) => res.json(await listSessions({ search: String(req.query.search || '') })));
app.delete('/api/sessions/:id', admin, async (req, res) => { await deleteSession(req.params.id); res.status(204).end(); });
